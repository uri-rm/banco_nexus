...
def realizar_deposito(transaccion: Transaccion):
    ...
    nuevo_saldo = cuenta_doc["saldo"] + transaccion.monto
    db["cuentas"].update_one({"cuenta": transaccion.cuenta}, {"$set": {"saldo": nuevo_saldo}})
    t = transaccion.dict()
    t["saldo"] = nuevo_saldo
    db["transacciones"].insert_one(t)
    return {"message": "Depósito realizado exitosamente", "nuevo_saldo": nuevo_saldo}
...
def realizar_retiro(transaccion: Transaccion):
    ...
    t["saldo"] = nuevo_saldo
    db["transacciones"].insert_one(t)
    return {"message": "Retiro realizado exitosamente", "nuevo_saldo": nuevo_saldo}
